...
public class ItemForShow {
    private final WebDriver driver ;
    private final FluentWait<WebDriver> wait;

    public ItemForShow (WebDriver driver){
        this.driver=driver;
    ...
    public ItemForShow navigateToItemForShowPage() {
        int maxAttempt = 3;
        for (int attempt = 0; attempt < maxAttempt; attempt++) {
            try {
                driver.get(ConfigUtils.getInstance().getItemForShowPage());
                Thread.sleep(2500);
                return this;
            } catch (Exception e) {
