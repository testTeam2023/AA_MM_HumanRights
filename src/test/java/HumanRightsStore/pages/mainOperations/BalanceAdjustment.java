...
private final By selectAssert=By.xpath("//select[@name='Dtl_UnitID']/option[@value='28']");
    ...
    public BalanceAdjustment enterItemNum(String itemNumbers ) throws InterruptedException {
        ...
        for (int attempt = 0; attempt < maxAttempt; attempt++) {
            try {
                ...
                waitForVisibilityElement(selectAssert);
                Thread.sleep(7000);
                return this;
            } catch (Exception e) {
        ...
        for (int attempt = 0; attempt < maxAttempt; attempt++) {
            try {
                ...
                System.out.println("Retrying click on save btn " + e.getMessage());
                         navigateToBalanceAdjustmentrPage()
                        .selectSoreName("المستودع العام")
                        .scrollDownForAddItem()
                        .enterItemNum("11")
                        .enterAdjustmentQty("1")
                        .enterAdjustmentType("2")
                        .clickOnAddBtn();
            }
        }
